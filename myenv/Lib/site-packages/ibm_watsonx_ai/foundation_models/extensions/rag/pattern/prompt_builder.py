...
def estimate_tokens_count(
    text: str, word_to_token_factor: float = WORD_TO_TOKENS_FACTOR
) -> int:
    ...
    :param word_to_token_factor: Constant representing the average number of tokens per word in a text, used for
        approximating the token count, defaults to 1.5
    :type word_to_token_factor: float, optional

    :return: the count of the tokens in the text
    :rtype: int
    """
    words = text.split()
    return int(len(words) * word_to_token_factor)
...
def build_prompt(
    ...
    word_to_token_factor: float = WORD_TO_TOKENS_FACTOR,
) -> str:
    ...
    selected_reference_documents = _select_reference_documents(
        ...
        word_to_token_factor=word_to_token_factor,
    )
...
def _select_reference_documents(
    ...
    available_input_tokens = (
        ...
        - estimate_tokens_count(prompt_template_text, word_to_token_factor)
        - estimate_tokens_count(question, word_to_token_factor)
        ...
        + estimate_tokens_count(RAGPattern.QUESTION_PLACEHOLDER, word_to_token_factor)
        + estimate_tokens_count(
            RAGPattern.REFERENCE_DOCUMENTS_PLACEHOLDER, word_to_token_factor
        )
    )
    ...
    for reference_document in reference_documents:
        ...
        tokens_required_for_reference_document = (
            estimate_tokens_count(reference_document, word_to_token_factor) + 1
        )
