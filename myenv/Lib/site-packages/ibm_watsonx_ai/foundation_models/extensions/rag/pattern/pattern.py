...
class RAGPattern:
    ...
    def __init__(
        ...
        *,
        ...
        word_to_token_factor: float = 1.5,
        **kwargs: Any,
    ) -> None:
        ...
        :param word_to_token_factor: Constant representing the average number of tokens per word in a text, used for approximating the token count, defaults to 1.5
        ...
        **Example:**
        ...
        WMLResource._validate_type(
            word_to_token_factor, "word_to_token_factor", [int, float], False, True
        )
        ...
        if word_to_token_factor <= 0:
            raise ValueError(
                "The value of 'word_to_token_factor' must be strictly positive, exceeding 0."
            )
        else:
            self.word_to_token_factor = word_to_token_factor
        self.kwargs = kwargs
    ...
    def _default_inference_function_params(self) -> dict:
        ...
        from ibm_watsonx_ai.foundation_models.extensions.rag.pattern.prompt_builder import (
            WORD_TO_TOKENS_FACTOR,
        )

        if self.word_to_token_factor != WORD_TO_TOKENS_FACTOR:
            default_params |= {"word_to_token_factor": self.word_to_token_factor}
